...
- Enviar variables extraídas a webhooks para su automatización
...
<AccordionGroup>
  ...
  <Accordion title="Variable Examples" icon="lightbulb">
    ...
    - `estado_de_la_resolución` (Verdadero/Falso): ¿Se resolvió el problema?
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Webhook URL" icon="link">
    ...
    **Ejemplo:** `https://automate.conecta2.ai/api/v1/webhooks/your-webhook-id`
...
El webhook recibirá una carga útil JSON completa con los detalles de la llamada, las variables extraídas y la transcripción. Para consultar la documentación completa de la API, visita, [Post-Call Webhook API](/api-reference/webhooks/post-call-webhook).
...
<AccordionGroup>
  <Accordion title="Call Information" icon="phone">
    ...
    - **teléfono_del_cliente:** Número de teléfono del cliente (formato E.164)
    - **teléfono_del_asistente:** Número de teléfono del asistente
    - **duración:** Duración de la llamada en segundos
    - **estado:** Estado de la llamada (iniciada, sonando, ocupada, en curso, finalizada, completada, finalizada por el cliente, finalizada por el asistente, sin respuesta, fallida)
    - **fecha_de_creación/fecha_de_finalización:** Marcas de tiempo ISO 8601
  </Accordion>
  
  <Accordion title="Extracted Variables" icon="code">
    - **variables_extraídas:** Datos extraídos por la IA basados ​​en la configuración del esquema posterior a la llamada (consulte la sección [Variables posteriores a la llamada](#post-call-variables) más arriba) 
    - **variables:** Variables pasadas antes de la llamada (consulte [Variables de llamada](/ai-assistants/settings/prompt-and-tools#call-variables))
...
{
  "id": 12345,
  "teléfono_del_cliente": "+1234567890",
  ...
  "recording_url": "https://app.conecta2.ai/storage/recordings/call-12345.mp4",
...
**Puntos finales de la API** cree receptores personalizados para datos de webhook
